...
import { listarPendentes } from "@/services/certificado.service";
...
export function DashboardMetricasPage() {
  ...
  useEffect(() => {
    const carregar = async () => {
      try {
        // Busca o dashboard geral e a fila de pendentes em paralelo
        const [dashRes, pendentesRes] = await Promise.all([
          api.get("/api/dashboard").then(r => r.data).catch(() => null),
          listarPendentes().catch(() => []),
        ]);

        const pendentesLista = Array.isArray(pendentesRes)
          ? pendentesRes
          : (pendentesRes?.certificados ?? pendentesRes?.pendentes ?? []);

        // Monta o objeto de dados unificando as duas fontes
        setDados({
          totalAlunos:          dashRes?.totalAlunos          ?? dashRes?.alunos          ?? "—",
          pendentes:            pendentesLista.length,
          aprovados:            dashRes?.aprovados             ?? dashRes?.totalAprovados   ?? 0,
          reprovados:           dashRes?.reprovados            ?? dashRes?.totalReprovados  ?? 0,
          horasTotaisValidadas: dashRes?.horasTotaisValidadas  ?? dashRes?.horasValidadas   ?? 0,
          distribuicaoPorArea:  dashRes?.distribuicaoPorArea   ?? [],
        });
      } catch {
  ...
  const taxaAprovacao  = totalValidados > 0
    ? `${Math.round((d.aprovados / totalValidados) * 100)}%`
    : "—";
  const mediaHoras = d.totalAlunos && typeof d.totalAlunos === "number" && d.totalAlunos > 0
    ? `${Math.round(d.horasTotaisValidadas / d.totalAlunos)}h`
    : "—";
